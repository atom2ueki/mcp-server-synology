...
class TestRealDownloadStation:
    ...
    def test_common_destinations(self, download_station):
        ...
        print(
            f"✅ Existing destinations: {', '.join(existing_dests) if existing_dests else 'None found'}"
        )
    ...
    def test_create_download_task(self, download_station, sample_download_url, test_destination):
        ...
        try:
            ...
            result = download_station.create_task(
                uri=sample_download_url, destination=test_destination
            )
            ...
            if any(
                keyword in error_msg.lower() for keyword in ["permission", "forbidden", "access"]
            ):
                pytest.skip(f"Permission denied (expected): {error_msg}")
            elif any(
                keyword in error_msg.lower() for keyword in ["destination", "folder", "directory"]
            ):
                pytest.skip(f"Destination issue (may be expected): {error_msg}")
            elif any(
                keyword in error_msg.lower() for keyword in ["network", "connection", "timeout"]
            ):
