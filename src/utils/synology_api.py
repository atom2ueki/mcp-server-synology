...
class SynologyAPIClient:
    ...
    def request(
        ...
        Args:
            ...
            timeout: Seconds to wait for a response, or None to use the
                client's request_timeout (see SynologyAPIClient.__init__).
        ...
        result = self._do_request(
            api, method, version, extra_params, use_post, effective_timeout
        )
    ...
    def post_stream(
        ...
        # Same single-retry session recovery as request(): 106/107/119 mean the
        # DSM session is gone, and the stream call is retried once after a
        # relogin. `attempted_sid` is captured before the call for the same
        # concurrent-caller reason as in request().
        error = result.get("error")
        code = error.get("code") if isinstance(error, dict) else None
        if not result.get("success") and code in SESSION_EXPIRED_CODES:
            new_sid, new_token = _try_relogin(self.base_url, attempted_sid)
