...
- Windows ACL enforcement on `settings.json` before load, verifying the owner SID via pywin32. The Windows branch now dispatches on `os.name == "nt"` before the POSIX guard that had made it dead code; a missing pywin32 skips the check with a warning (fail-open, matching documented behaviour) while win32 runtime errors fail closed. (#83)
...
- Tool dispatch now runs off a registry. A 50-branch `if/elif` chain in `_dispatch_tool` and a parallel 300-line `_get_tool_definitions()` were replaced by a single dict of ~79 registered tools, so a tool's schema and its handler can no longer drift apart. (#82)
